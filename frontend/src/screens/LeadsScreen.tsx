...
import { Avatar, Card, Chip, EmptyState, fmtAED, GhostButton, PrimaryButton, timeAgo } from '@/src/components/ui';
              ...
              {expanded[s.key] && (
                <View style={{ gap: spacing.sm, marginTop: spacing.sm }}>
                  {items.length === 0 ? (
                    ...
                    items.map((l) => <LeadRow key={l.id} lead={l} onPress={() => onOpenLead(l.id)} agents={state.agents} />)
                  )}
                </View>
              )}
            </View>
...
const LeadRow: React.FC<{ lead: Lead; onPress: () => void; agents: any[] }> = ({ lead, onPress, agents }) => {
...
const styles = StyleSheet.create({
  ...
  leadName: { color: colors.text, fontSize: fontSize.base, fontWeight: '800' },
  leadTime: { color: colors.textSubtle, fontSize: fontSize.xs },
  leadMeta: { color: colors.textMuted, fontSize: fontSize.xs, marginTop: 3 },
  leadFoot: { flexDirection: 'row', gap: 8, marginTop: 8, alignItems: 'center' },
  scorePill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  scoreText: { fontSize: fontSize.xs, color: colors.text, fontWeight: '700' },
  agentPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
